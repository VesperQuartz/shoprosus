...
export const CartMenuUI = makeAssistantToolUI<never, CartTableSelect[]>({
  toolName: "getUserCart",
  render: ({ status, result }) => {
    if (status.type === "incomplete" && status.error === "error") {
      return <div className="text-red-500">Failed to get items in cart</div>;
    }
    if (result && result?.length === 0) {
      return <div>You have no items in cart!</div>;
    }
    return match(status.type)
      ...
      .otherwise(() => {
        return <div className="text-red-500">Failed to get Cart item</div>;
      });
  },
});
